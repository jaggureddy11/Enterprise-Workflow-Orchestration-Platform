...
export interface TaskStepConfig {
  ...
  assignee: string; // userId or role name, supports context vars like {{context.employee.managerId}}
  slaHours: number;
...
export interface NotificationStepConfig {
  ...
  template: string;
  recipients?: string[];
  webhook?: string; // for Slack
  isTerminal?: boolean;
  nextStepOrder?: number;
}

export interface ConditionStepConfig {
  expression: string; // JSONPath expression e.g. "$.context.employee.department === 'ENGINEERING'"
